...
@app.post("/data-analyst-roles")
async def data_analyst_roles(file:UploadFile=File(...)):
    try:
        df=pd.read_csv(file.file, encoding="UTF-8")
    except UnicodeDecodeError:
    ...
    df['normalized_title'] = df['title'].apply(normalize_title)
    df = df.replace([np.inf, -np.inf], '').fillna('')
    # Filter for normalized title == "data analyst"
    analyst_normalized = normalize_title("data analyst")
    filtered = df[df['normalized_title'] == analyst_normalized]
    # Only return title and company
    results = filtered[["title", "company"]].to_dict(orient="records")
    return {"data_analyst_jobs": results}